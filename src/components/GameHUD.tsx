    ...
    <div className="absolute inset-0 pointer-events-none z-20 flex flex-col justify-between p-3 sm:p-5 select-none">
      ...
      <div className="flex items-start justify-between w-full">
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-baseline gap-2 bg-black/60 backdrop-blur-md px-3.5 py-1.5 rounded-xl border border-white/10 shadow-lg">
            <span className="text-xs text-white/60 font-semibold tracking-wider uppercase">Pontos</span>
            <span className="font-arcade text-2xl sm:text-3xl font-black text-[#FFD23F] drop-shadow-[0_0_12px_rgba(255,210,63,0.5)]">
              {score.toLocaleString()}
            </span>
          </div>

          <div className="flex items-center gap-1.5 px-3 py-1 bg-black/40 backdrop-blur-sm rounded-lg border border-white/5 w-fit">
        ...
        <div className="flex items-center gap-2 sm:gap-3 pointer-events-auto">
          {mode !== 'zen' && mode !== 'time_attack' && (
            <div className="flex items-center gap-1 bg-black/60 backdrop-blur-md px-3 py-2 rounded-xl border border-white/10 shadow-lg">
          ...
          <button
            id="btn-pause-game"
            onClick={onPause}
            className="w-10 h-10 rounded-xl bg-black/60 backdrop-blur-md border border-white/10 flex items-center justify-center text-white/80 hover:text-white hover:bg-white/10 active:scale-95 transition-all shadow-lg cursor-pointer"
      ...
      <div className="flex items-end justify-between w-full">
          ...
          {combo > 1 ? (
            <div className="flex items-center gap-2 bg-gradient-to-r from-amber-600/90 to-red-600/90 backdrop-blur-md px-3.5 py-1.5 rounded-xl border border-amber-300/40 shadow-[0_0_15px_rgba(255,100,0,0.4)] animate-pulse">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] text-yellow-200 font-bold tracking-wider leading-none">COMBO</span>
                <span className="font-arcade text-xl sm:text-2xl font-black text-white leading-none">
                  x{combo}
                </span>
              </div>
            </div>
        ...
        <div className="flex items-center gap-2 bg-black/50 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10">
          ...
          <span className="text-xs text-white/70 font-medium">
            {handConfidence > 0.35 ? 'Mão Detectada ✋' : 'Mova a mão / Toque'}
          </span>
        </div>
      </div>
    </div>
